...
QList<device> textExplainer::getDevList_windows(QString tmpStr)
{
    ...
    for(int i = 1; i < tmpList.size() ; i++)
    {
        ...
        qDebug() << "dev.state=" <<  dev.state;
        //qDebug() << "*tmplist[" << i << "] =" <<  tmpList[i];
        qDebug() << "Bug#01 is not here!3";
        if(dev.state == "[未响应]")
        {
...
bool textExplainer::explainError(QString err)
{
    ...
    else if(err.contains("could not start server"))
    {
        ...
        //noticeWidget *notWgt = new noticeWidget(nullptr);
        //notWgt->popUp("上次运行时疑似异常退出？您可以反馈此问题！");
    }
    ...
    {
        showMsgBox("错误"," · 未知错误！\n" + err +"\n · 若此处出现并不完整的报错或一大串包含 '.android.os'、'java' 等字样的错误，您可以视情况忽视它，并查阅另一条报错\n");
    }
